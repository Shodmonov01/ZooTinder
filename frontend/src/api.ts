...
export function mediaUrl(url?: string | null) {
  if (!url) return ''
  if (/^https?:\/\//i.test(url) && !url.includes('/uploads/') && !url.includes('/api/v1/media/')) {
    return url
  }
  const match = url.match(/\/(?:uploads|media)\/([^/?#]+)/)
  const path = match ? `${API_URL}/media/${match[1]}` : url.startsWith('/') ? url : `${API_URL}/media/${url}`
  const token = readTokens()?.accessToken
  if (!token) return path
  return `${path}${path.includes('?') ? '&' : '?'}access=${encodeURIComponent(token)}`
}
...
export async function api<T>(
  ...
  const timeoutMs = rest.body instanceof FormData ? 30_000 : 8_000
